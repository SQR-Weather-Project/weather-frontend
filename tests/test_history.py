...
def test_history(driver):
    ...
    option_ru = wait.until(
        EC.visibility_of_element_located((By.XPATH, "//div[text()='RU']"))
    )
    option_ru.click()

    city_select = wait.until(
        EC.presence_of_element_located(
            (By.XPATH, "//label[contains(., 'City')]/following-sibling::div")
        )
    )
    city_select.click()
    option_msc = wait.until(
        EC.visibility_of_element_located((By.XPATH, "//div[text()='Moscow']"))
    )
    option_msc.click()

    date_select = wait.until(
        EC.presence_of_element_located(
            (By.XPATH, "//label[contains(., 'Date')]/following-sibling::div")
        )
    )
    date_select.click()
    ...
    time_select = wait.until(
        EC.presence_of_element_located(
            (By.XPATH, "//label[contains(., 'Time')]/following-sibling::div")
        )
    )
    time_select.click()
    option_all = wait.until(
        EC.visibility_of_element_located((By.XPATH, "//div[text()='All']"))
    )
    option_all.click()
    ...
    element = WebDriverWait(driver, 10).until(
        EC.text_to_be_present_in_element(
            (By.CSS_SELECTOR, "div[data-testid='stCaptionContainer'] > p"),
            expected_text,
        )
    )

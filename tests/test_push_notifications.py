...
def test_push_notifications():
    ...
    fill_title = wait.until(EC.element_to_be_clickable((
        By.XPATH,
        "//label[contains(., 'Title')]/following-sibling::div//input"
    )))
    fill_title.send_keys("New Notification")

    fill_body = wait.until(EC.element_to_be_clickable((
        By.XPATH,
        "//label[contains(., 'Body')]/following-sibling::div//input"
    )))
    fill_body.send_keys("New Body")

    icon_label = wait.until(EC.element_to_be_clickable((
        By.XPATH,
        "//label[.//p[text()='Icon:']]"
    )))
    icon_label.click()

    push_button = driver.find_element(
        By.XPATH,

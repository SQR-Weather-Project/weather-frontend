...
def test_notification_settings():
    ...
    slider = wait.until(EC.presence_of_element_located((
        By.CLASS_NAME, "stSlider")))
    move_slider_to_value(driver, slider, 30)

    param_select = wait.until(EC.presence_of_element_located((
        By.XPATH,
        "//label[contains(., 'choose a parameter')]/following-sibling::div"
    )))
    param_select.click()
    option_temp = wait.until(EC.visibility_of_element_located((
        By.XPATH,
    ...
    wait.until(EC.presence_of_element_located((
        By.XPATH,
        "//label[contains(., 'choose a filter')]/following-sibling::div"
    )))
    ...
    filter_select.click()
    option_above = wait.until(EC.visibility_of_element_located((
        By.XPATH,
        "//div[text()='above']")))
    option_above.click()
    ...
    city_select = driver.find_elements(
        By.XPATH,
        "//label[contains(., 'Select a city')]/following-sibling::div"
    )[-1]
    city_select.click()
    option_kazan = wait.until(EC.visibility_of_element_located((
        By.XPATH,
        "//div[text()='Kazan']")))
    option_kazan.click()
...
def move_slider_to_value(driver, slider_element, target_value):
    slider_handle = slider_element.find_element(
        By.XPATH,
        ".//*[@role='slider']")

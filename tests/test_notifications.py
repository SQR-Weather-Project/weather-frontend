...
def test_notification_settings(driver):
    ...
    slider = wait.until(EC.presence_of_element_located((By.CLASS_NAME,
                                                        "stSlider")))
    ...
    param_select = wait.until(
        EC.presence_of_element_located(
            (
                By.XPATH,
                "//label[contains"
                "(., 'choose a parameter')]/following-sibling::div",
            )
        )
    )
    param_select.click()
...
def move_slider_to_value(driver, slider_element, target_value):
    slider_handle = slider_element.find_element(By.XPATH,
                                                ".//*[@role='slider']")
    ...
    action.click_and_hold(slider_handle).\
        move_by_offset(x_offset, 0).release().perform()

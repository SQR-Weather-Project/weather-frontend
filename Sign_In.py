...
if st.button("Войти через Telegram"):
    try:
        response = requests.post(f"{BACKEND_URL}/user/login", params={"token": token, "callback_url": f"{FRONTEND_URL}/login_success"})
        if response.status_code == 200:
            st.success("Перейдите в Telegram бота для продолжения авторизации.")

            tg_url = f"https://t.me/{TG_BOT_USERNAME}?start={token}"

            st.markdown(
                f'[Открыть Telegram-бота](https://t.me/{TG_BOT_USERNAME}?start={token})',
